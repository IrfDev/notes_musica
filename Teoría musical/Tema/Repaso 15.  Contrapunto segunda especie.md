...
```mermaid
flowchart TD
    A["Contrapunto Segunda Especie<br/>(Dos notas contra una)"] --> B["1. Reglas de Intervalos"]
    A --> C["2. Reglas de Movimiento"]
    A --> D["3. Reglas Melódicas"]
    A --> E["4. Características Especiales"]

    B --> B1["Tiempos Fuertes<br/>Solo Consonancias"]
    B --> B2["Tiempos Débiles<br/>Consonancias o Disonancias"]

    B1 --> B1A["Inicio: 5ª u 8ª<br/>con silencio"]
    B1 --> B1B["Final: 5ª u 8ª"]

    B2 --> B2A["Disonancias solo<br/>por grado conjunto"]
    B2 --> B2B["Disonancias como<br/>notas de paso"]

    C --> C1["Contrario<br/>(Preferido)"]
    C --> C2["Oblicuo<br/>(Aceptable)"]
    C --> C3["Directo<br/>(Con precaución)"]
    C --> C4["Paralelo<br/>(Limitado)"]

    D --> D1["Saltos compensados<br/>con grado conjunto"]
    D --> D2["Evitar saltos<br/>consecutivos"]
    D --> D3["Rango máximo<br/>de una octava"]

    E --> E1["Inicio con silencio<br/>en primer tiempo"]
    E --> E2["Movimiento constante<br/>en negras"]
    E --> E3["Quintas y octavas<br/>ocultas"]
```
...
```mermaid
flowchart TD
  A["Nota del Cantus Firmus (compás i)"] --> B{Tiempo Fuerte<br/>1ª blanca}
  B -->|Solo consonancia| C[Elige 3ª/6ª/5ª/8ª]
  C --> D{Tiempo Débil<br/>2ª blanca}
  D -->|Consonancia| E[Consonancia de paso/vecina consonante]
  D -->|Disonancia| F[SOLO nota de paso]
  F --> G[Debe entrar por grado conjunto]
  G --> H[Debe salir por grado conjunto]
  H --> I[El siguiente tiempo fuerte debe ser consonante]
```
