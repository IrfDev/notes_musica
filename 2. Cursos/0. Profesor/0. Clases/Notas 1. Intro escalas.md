Las notas tienen tonos para arriba o para abajo, los cuáles son tonos o semitonos

Por la estructura de la guitarra todas las notas entre sí tienen un tono, excepto por si. y fa

Los acordes se arman a partir de intervalos específicos entre notas. 


The graphic representation of tones and semitones, goes as follow:


```music-abc
X:1
T:Escala de Do y distancias
M:4/4
L:1/4
K:C
"(C)DO"C "(D)RE"D "(E)MI"E "(F)FA"F | "(G)SOL"G "(A)LA"A "(B)SI"B "(C)DO"c |]
w: +1 +1 +0.5 +1 +1 +1 +0.5 llegada
```



# Scales
Scales are a set of notes following a specific pattern/formula of tones and semitones
# C major scale:
All of the notes starting from C to D
And applying the following pattern: 
1. Base (Tónica)
2. Base + 2 (Tercera)
	1. Sí se suman 3 semitonos es menor 
	2. Sí suman 4 semitonos es mayor 
3. Base + 4 (quinta)
	1. Sí se suman 7 semitonos es justa [checar por tonos]
	2. Sí se suman 8 semitonos es aumentada
	3. Sí se suman 6 semitonos es disminuida


```music-abc
X:1
T:C chord
M:4/4
K:C
C:Trad.
% This is a C Major chord progression with all notes spelled out
% Each measure contains the root, third, and fifth of each chord
|:"C"C2 "C+2=E"E2 "C+4=G"G2 
w:C E G
```
De C (tónica) a E (tercera) son 4 semitonos, por lo tanto es mayor

```music-abc
X:1
T: D chord
M:4/4
K:C
C:Trad.
% This is a C Major chord progression with all notes spelled out
% Each measure contains the root, third, and fifth of each chord
|:"D"D2 "D+2=F"F2 "D+4=A"A2 
w:D F A
```
De D (tónica) a F (tercera) son 3 semitonos, por lo tanto es menor

```music-abc
X:1
T: E Chord
M:4/4
K:C
C:Trad.
% This is a C Major chord progression with all notes spelled out
% Each measure contains the root, third, and fifth of each chord
|:| E2 "E+2=G"G2 "E+4=B"B2 
w: E G B
```
De E (tónica) a G (tercera) son 3 semitonos, por lo tanto es menor

```music-abc
X:1
T: F Chord
M:4/4
K:C
C:Trad.
% This is a C Major chord progression with all notes spelled out
% Each measure contains the root, third, and fifth of each chord
|:|"F"F2 "F+2=A"A2 "F+4=C"C2
w: F A C
```

De F (tonica) a A son 4 semitonos por lo tanto es mayor


```music-abc
X:1
T: Bdis Chord
M:4/4
K:C
C:Trad.
% This is a C Major chord progression with all notes spelled out
% Each measure contains the root, third, and fifth of each chord
|:|"B" B, "B+2=D"D "B+4=F"F
w: B D F
```
De B a D son 3 semitonos (1.5 tonos) por lo tanto es menor
De B (tónica) a F (5ta) son 6 semitonos, por lo tanto es disminuido
(Regularmente 5ta dis va con 3ra menor)


## Preguntas para Fabs: 
- Por 10ma vez, cómo se arma un acorde en la guitarra teniendo las notas?
	- Los acordes se arman poniendo la tónica como base y de ahí encontrando las notas que determinaron nuestro acorde a lo lardo de la guitarra
- Guía para poder comenzar con otras escalas

## Siguiente tema: 

Fundamental: 
La tónica tiene que ser grave
voicing es donde ordenas tus notas
La misma tónica 


## Objetivos marzo: 
Poder tocar de principio a fin: 
- Canciones:
	- Gagging Order - Radiohead
- Escalas: 
	- Armonizar y tocar todas las escalas mayor
	- Poder tocar la escala de C mayor en todas las posiciones
	- Poder tocar la escala de 7mas en todas las posiciones 
- Poder usar al menos dos modos (compas) del metrónomo

